...
export const filterExcludedProducts = (data: any) => {
  return data
    ...
    .filter((e: any) => !e.categoryHierarchy?.includes('Kozmetik'))
    .filter((e: any) => !e.categoryHierarchy?.includes('Cinsel'))
    .filter((e: any) => !e.categoryHierarchy?.includes('Sıvı'))
    .filter((e: any) => !e.categoryHierarchy?.includes('Deterjan'))
    .filter((e: any) => !e.categoryHierarchy?.includes('Mobilya'))
    .filter((e: any) => !e.categoryHierarchy?.includes('Dambıl'))
    .filter((e: any) => !e.categoryHierarchy?.includes('Fitness'))
    .filter((e: any) => !e.name?.toLowerCase().includes('sıvı'))
    .filter((e: any) => !e.name?.toLowerCase().includes('şurup'))
    .filter((e: any) => !e.name?.toLowerCase().includes('surup'))
    .filter((e: any) => !e.name?.toLowerCase().includes('şurub'))
    .filter((e: any) => !e.name?.toLowerCase().includes('surub'))
    .filter((e: any) => !e.name?.toLowerCase().includes('sprey'))
    .filter((e: any) => !e.name?.toLowerCase().includes('spray'))
    .filter((e: any) => !e.name?.toLowerCase().includes('damla'))
    .filter((e: any) => !e.name?.toLowerCase().includes('içecek'))
    .filter((e: any) => !e.name?.toLowerCase().includes('1 lt'))
    .filter((e: any) => !e.name?.toLowerCase().includes('1000 ml'))
    .filter((e: any) => !e.name?.toLowerCase().includes('500 ml'))
    .filter((e: any) => !e.name?.toLowerCase().includes('likit'))
    .filter((e: any) => !e.name?.toLowerCase().includes('shot'))
...
export const transformData = (data: any): ShopProductListItemData => {
  const id = `${encryptNum(data.id).toString()}-${encryptNum(data.merchantId)}-${encryptNum(
    data.itemNumber
  )}`;
  ...
  const product: ShopProductListItemData = {
    ...
    breadcrumbs: data.categoryHierarchy,
  };
...
export const getFilters = async (params: URLSearchParams, locale: Locale) => {
  try {
    let url = `https://public.trendyol.com/discovery-web-searchgw-service/v2/api/aggregations/sr?channelId=1&${params}`;
