...
const trendyolRecommendationEngine: RecommendationEngine = async ({
  brandId: encryptedBrandId,
  productId: encryptedProductId,
}) => {
  try {
    const webBrandId = decryptNum(encryptedBrandId).toString();
    const pid = decryptNum(encryptedProductId).toString();
    const url = new URL(
      `https://apigw.trendyol.com/discovery-web-websfxproductrecommendation-santral/api/v1/product/${pid}/recommendation?size=24&page=0&stamp=TypeA&storefrontId=1&isGoogleBotUser=false&channelId=1`
    );
...
export default trendyolRecommendationEngine;
